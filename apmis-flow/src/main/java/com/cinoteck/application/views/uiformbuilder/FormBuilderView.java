...
public class FormBuilderView extends FormLayout {
	...
	public FormBuilderView() {
		...
		add(formType, formId, caption, important, optionsKey, optionsValue, expression, dependingOn, dependingOnValues,
				styles, constraints, min, max, errorMessage, generateJson, clearTextArea, jsonContainer);
	}

	public void configure() {

		optionsKey.setHelperText("Enter options keys in a comma seperated format like so Urban, Rural");
		optionsValue.setHelperText("Enter options values in a comma seperated format like so Urban, Rural");
	...
	public void valueChange() {
		...
		optionsKey.addValueChangeListener(e -> {

		});
		
//		 "options":[{ "key":"yes", "caption":"Yes" },{ "key":"no", "caption":"No"}]
		optionsValue.addValueChangeListener(e -> {
		...
		constraints.addValueChangeListener(e -> {
			...
			if (e.getValue().toString().toLowerCase().equals("expression")) {
	...
	public void generateJson() {
		...
		if (jsonContainer.getValue() != null) {

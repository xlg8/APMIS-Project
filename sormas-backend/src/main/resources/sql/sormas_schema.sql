...
AS SELECT areas.name AS area, 
    ...
    max( 
        CASE 
            ...
            CASE 
                WHEN (jsonmeta.value ->> 'type'::text) = 'number'::text OR (jsonmeta.value ->> 'type'::text) = 'decimal'::text OR (jsonmeta.value ->> 'type'::text) = 'range'::text THEN (jsondata.value ->> 'value'::text)::numeric 
                ELSE 0::numeric 
            END 
            ELSE 1::numeric 
        END) AS day4, 
   ...
   FROM campaignformdata 
    ...
    LATERAL json_array_elements(campaignformdata.formvalues) jsondata(value), 
    LATERAL json_array_elements(campaignformmeta.campaignformelements) jsonmeta(value) 
  WHERE (jsondata.value ->> 'value'::text) IS NOT NULL AND ((jsondata.value ->> 'id'::text) = ANY (ARRAY['Admin_day4-readonly'::text, 'Admin_day2-readonly'::text])) AND (jsondata.value ->> 'id'::text) = (jsonmeta.value ->> 'id'::text) AND campaignformmeta.formcategory::text = 'ADMIN'::text 

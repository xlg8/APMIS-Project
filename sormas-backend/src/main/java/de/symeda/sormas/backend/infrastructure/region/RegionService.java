...
public class RegionService extends AbstractInfrastructureAdoService<Region> {
		...
		if (criteria.getNameEpidLike() != null) {
			...
			for (String textFilter : textFilters) {
				...
				Predicate likeFilters = cb
						.or(CriteriaBuilderHelper.unaccentedIlike(cb, from.get(Region.NAME), textFilter));// ,
		...
		if (criteria.getRelevanceStatus() != null) {
			if (criteria.getRelevanceStatus() == EntityRelevanceStatus.ACTIVE) {
				filter = CriteriaBuilderHelper.and(cb, filter,
						cb.or(cb.equal(from.get(Region.ARCHIVED), false), cb.isNull(from.get(Region.ARCHIVED))));
	...
	public Set<Region> getByReferenceDto(Set<RegionReferenceDto> region) {
		Set<Region> regions = new HashSet<Region>();
		for (RegionReferenceDto com : region) {
			if (com != null && com.getUuid() != null) {
				Region result = getByUuid(com.getUuid());
				if (result == null) {

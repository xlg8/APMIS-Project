...
//			cq.where(filter, filterx);
	...
	public List<DistrictDto> getAllActiveAsReferenceAndPopulation(Long regionId, CampaignDto campaignDt) {
		String queryStringBuilder = "select a.name, p.population, a.id, ar.uuid as umid, a.uuid as uimn, p.selected, p.modality, p.districtstatus, p.agegroup from district a\n"
				...
				+ "group by a.name, p.population, a.id, ar.uuid, a.uuid, p.selected, p.modality, p.districtstatus, p.agegroup";

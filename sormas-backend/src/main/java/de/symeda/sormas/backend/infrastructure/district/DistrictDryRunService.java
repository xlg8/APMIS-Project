...
@Stateless
@LocalBean
public class DistrictDryRunService extends AbstractInfrastructureAdoService<DistrictDryRun> {

	@EJB
	private CountryFacadeEjbLocal countryFacade;

	public DistrictDryRunService() {
		super(DistrictDryRun.class);
	}

	public List<DistrictDryRun> getAllActiveByRegion(Region region) {

		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<DistrictDryRun> cq = cb.createQuery(getElementClass());
		Root<DistrictDryRun> from = cq.from(getElementClass());
		cq.where(cb.and(createBasicFilter(cb, from), cb.equal(from.get(DistrictDryRun.REGION), region)));
		cq.orderBy(cb.asc(from.get(DistrictDryRun.NAME)));

		return em.createQuery(cq).getResultList();
	}

	public List<DistrictDryRun> getAllActiveByArea(Area area) {

		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<DistrictDryRun> cq = cb.createQuery(getElementClass());
		Root<DistrictDryRun> from = cq.from(getElementClass());

		Path<Area> regionJoin = from.join(DistrictDryRun.REGION, JoinType.LEFT);
	...
	public List<DistrictDryRun> getAllActiveByServerCountry() {
	...
	public List<DistrictDryRun> getByName(String name, Region region, boolean includeArchivedEntities) {
	...
	public List<DistrictDryRun> getByExternalId(Long ext_id, Region reg_ext_id, boolean includeArchivedEntities,
			int notUsed) {
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<DistrictDryRun> cq = cb.createQuery(getElementClass());
		Root<DistrictDryRun> from = cq.from(getElementClass());
	...
	public List<DistrictDryRun> getByExternalId(Long externalId, boolean includeArchivedEntities) {
		return getByExternalId(externalId, District.EXTERNAL_ID, includeArchivedEntities);
	}

	@SuppressWarnings("rawtypes")
	@Override
	public Predicate createUserFilter(CriteriaBuilder cb, CriteriaQuery cq, From<?, DistrictDryRun> from) {
		// no filter by user needed
		return null;
	}

	public Predicate buildCriteriaFilter(DistrictCriteria criteria, CriteriaBuilder cb, Root<DistrictDryRun> from) {
		Join<DistrictDryRun, Region> region = from.join(DistrictDryRun.REGION, JoinType.LEFT);
	...
	public Set<DistrictDryRun> getByReferenceDto(Set<DistrictReferenceDto> district) {
		Set<DistrictDryRun> districts = new HashSet<DistrictDryRun>();
		for (DistrictReferenceDto com : district) {
			if (com != null && com.getUuid() != null) {
				DistrictDryRun result = getByUuid(com.getUuid());
				if (result == null) {
